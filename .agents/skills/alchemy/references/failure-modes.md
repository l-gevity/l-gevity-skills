...
| A decision lives in a standalone document while the issue that raised it records nothing | Documentation architecture companion | Run `zero-copy-requirements`; close the decision on the issue that raised it and cite that issue where the decision is needed |
| A hand-maintained status page, changelog, or decision log drifts from the artifacts it summarizes | Documentation architecture companion | Run `zero-copy-requirements`; regenerate it from the primitives or delete it — a second authority cannot be kept in sync by discipline |
| An advisory is suppressed with a comment and no owner, grade, or reopening date | Dependency lifecycle companion | Run `dependency-lifecycle`; grade reachability, then record the acceptance with an owner and an expiry, or choose another response |
| A dependency bump dispatched `SKIP` although it moved a default, a floor, or a serialized shape | Dependency lifecycle companion — bump classified by version number | Classify by contract, not by the number; a contract-changing bump runs the gate it touches |
| A production failure was first reported by a user or a downstream team | Observability design companion | Run `observability-design`; the residual risk had no detection path, which is unobserved rather than accepted |
| Alerts fire regularly and change nothing, so the one that matters is acknowledged with the rest | Observability design companion — no prune rule | Delete or re-derive each alert from the objective it defends; attention is a shared budget |
